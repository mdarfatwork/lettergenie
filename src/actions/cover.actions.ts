...
export const editCoverLetter = actionClient
  .inputSchema(editCoverLetterSchema)
  .action(async ({ parsedInput }) => {
    try {
      const { userId } = await auth();
      ...
      const { id, jobTitle, companyName, jobDescription, customInstructions } =
        parsedInput;
...
`;
      ...
      // Save cover letter to database
      const savedCoverLetter = await db.coverLetter.update({
        where: { id },
        data: {
          clerkId: userId,
          profileId: userProfile?.id,
      ...
      throw new Error("Failed to edit cover letter. Please try again.");
    }
  });
